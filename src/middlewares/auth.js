...
const generalAccessToken = (data, exprie) => {
  return new Promise((resolve, reject) => {
    try {
      if (data) {
        const access_token = jwt.sign(data, privateKey, {
          algorithm: "RS256",
          expiresIn: exprie,
        });
...
const generalRefreshToken = (data, exprie) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (data) {
        const refresh_token = jwt.sign(data, privateKey, {
          algorithm: "RS256",
          expiresIn: exprie,
        });
...
const verifyRefreshToken = async (req, res, next) => {
  try {
    ...
    const refresh_token_server = await getDataRedisService("refresh_token");
    if (refresh_token_server === refresh_token_client) {
      jwt.verify(refresh_token_client, publicKey, (err, user) => {
        if (err)
          res.status(401).json({
            errCode: 2,
            message: err.message,
...
const verifyAccessToken = async (req, res, next) => {
  ...
  jwt.verify(access_token, publicKey, async (err, user) => {
    if (user && user.roleId) {

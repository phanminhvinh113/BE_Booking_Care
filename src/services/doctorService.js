  ...
  return new Promise(async (res, rej) => {
    try {
      if (!doctorId) {
        ...
        infoDoctor.image = Buffer.from(infoDoctor.image, "base64").toString(
          "binary"
        );
        res({
          errCode: 0,
    ...
    try {
      if (!data.arrSchedule || !data.arrSchedule.length) {
        ...
        if (existSchedule && !!existSchedule.length) {
          ...
          const newSchedule = _.differenceWith(
            schedule,
            existSchedule,
            (a, b) => {
    ...
    try {
      if (!doctorId) {
        ...
        const feedbacks = await db.Evaluate.findAll({
          ...
          attributes: {
            exclude: ["createdAt", "updatedAt", "token", "id", "doctorId"],
          },
          include: [
            {
              model: db.User,
